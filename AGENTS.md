...
### 🔴 Конфигурация — только с подтверждением

**Запрещено менять без явного подтверждения Макса:**
- Cron-джобы (модель, payload, schedule, delivery)
- Переменные окружения (GitHub Actions variables, .env)
- Конфигурацию gateway, MCP-серверов
- Настройки моделей для кронов
- Всё что Макс ранее настраивал лично

Перед любым изменением конфигурации:
1. `memory_recall` по теме — проверить нет ли противоречащих фактов
2. Если противоречия есть — поднять флаг Максу, НЕ выбирать одну версию
3. Явно спросить разрешение с объяснением что и зачем меняешь

### 🏷️ Устаревшие уроки

Если «урок» или факт опровергнут:
- Обновить содержимое через `memory_update` — дописать что опровергнуто и когда
- Не удалять — история нужна
- В AGENTS.md не дублировать (правила здесь — только актуальные)
